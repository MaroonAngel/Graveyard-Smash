...
public class JackEntity extends HostileEntity implements RangedAttackMob {
    ...
    public JackEntity(EntityType<? extends JackEntity> entityType, World world) {
    ...
    protected void initGoals() {
        this.goalSelector.add(0, new JackEntity.DescendAtHalfHealthGoal());
    ...
    public boolean damage(DamageSource source, float amount) {
        if (this.isInvulnerableTo(source)) {
            return false;
        } else if (source != DamageSource.DROWN && !(source.getAttacker() instanceof JackEntity)) {
    ...
    public static DefaultAttributeContainer.Builder createJackAttributes() {
        return HostileEntity.createHostileAttributes().add(EntityAttributes.GENERIC_MAX_HEALTH, 150.0D).add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.6000000238418579D).add(EntityAttributes.GENERIC_FOLLOW_RANGE, 40.0D).add(EntityAttributes.GENERIC_ARMOR, 4.0D);
    }
    ...
    static {
        TRACKED_ENTITY_ID_1 = DataTracker.registerData(JackEntity.class, TrackedDataHandlerRegistry.INTEGER);
        TRACKED_ENTITY_ID_2 = DataTracker.registerData(JackEntity.class, TrackedDataHandlerRegistry.INTEGER);
        TRACKED_ENTITY_ID_3 = DataTracker.registerData(JackEntity.class, TrackedDataHandlerRegistry.INTEGER);
        TRACKED_ENTITY_IDS = ImmutableList.of(TRACKED_ENTITY_ID_1, TRACKED_ENTITY_ID_2, TRACKED_ENTITY_ID_3);
        INVUL_TIMER = DataTracker.registerData(JackEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    class DescendAtHalfHealthGoal extends Goal {
        ...
        public boolean canStart() {
            return JackEntity.this.getInvulnerableTimer() > 0;
        }
    }
}
